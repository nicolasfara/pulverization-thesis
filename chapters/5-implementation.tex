...
steps~\cite{DBLP:phd/ch/Doeraene18}:
\begin{itemize}
...
companies. Year to year several improvements were made to reach a high level of performance and stability~\cite{DBLP:conf/dls/MarrDM16}.
...
\begin{figure}[ht]
	...
	\caption[Kotlin expect actual mechanism]{Kotlin multiplatform \texttt{expect/actual} mechanism.}
...
hell''~\cite{DBLP:conf/esem/GallabaMB15}. Futures and promises are a step above callback-based computations, working as special proxies for not-yet-completed results
...
\cite{DBLP:conf/oopsla/ElizarovBAU21}. Coroutines have been classified using the following axes, where implementations may differ:
...
asymmetric~\cite{DBLP:conf/oopsla/ElizarovBAU21}.
...
Kotlin coroutines are built upon the following goals~\cite{DBLP:conf/oopsla/ElizarovBAU21}:
\begin{itemize}
...
The formulation presented in the original paper~\cite{DBLP:journals/fi/CasadeiPPVW20} defines the sensors module as \textit{``a set $\sigma$ of logical sensors''};
...
The interface is generic in a type variable \texttt{T} that represents the type of data collected by the sensor and implements the
\texttt{Initializable} interface to model the ability of initialization of the sensor.

\lstinputlisting[
	float=!ht,
	language=Kotlin,
	caption={[Implementation of sensors container]Implementation of the \texttt{SensorsContainer} using inline methods and \texttt{KClass}.},
...
\begin{figure}[ht]
	...
	\caption[Package diagram of the platform module]{Package diagram of the \texttt{platform} module showing the relationship between the packages.}
...
\lstinputlisting[
	float=ht,
	language=Kotlin,
	caption={Communicator interface defined by the framework.},
...
\lstinputlisting[
	float=ht,
	language=Kotlin,
	caption={[Implementation of component reference interface]Implementation of the \texttt{ComponentRef} interface.},
...
\lstinputlisting[
	float=ht,
	language=Kotlin,
	caption={[Implementation of component reference that do nothing]Implementation of the \texttt{ComponentRef} interface that does nothing.},
...
RabbiMQ is an open source \textbf{message broker} written in Erlang that implements the \textbf{AMQP} protocol~\cite{DBLP:journals/internet/Vinoski06e}.
