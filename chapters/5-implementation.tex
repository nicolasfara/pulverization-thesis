...
This section provides an overview of the technologies used in the framework in the various modules. Key libraries and mechanisms for the operation of
the framework will be covered, reporting only those that are common to all modules; technologies and dependencies specific to each module will be
...
recent years, either as structured concurrency libraries or as a built-in language feature~\cite{alan-struct-concurr}.
...
The~\Cref{lst:example-suspending-function} shows an example of a suspending function, which is called from another suspending function.
...
For those reasons, \emph{coroutine} and \emph{Flow} are the main tools where the framework is based, providing a simple and efficient way to
implement asynchronous behaviors.
...
In this example, a data class \texttt{MySimpleClass} is defined, and a module \texttt{myModule} is defined to provide an instance of the class as a
singleton. The \texttt{startKoin} function is then used to start Koin and use the defined module. Finally, the dependency is injected into
