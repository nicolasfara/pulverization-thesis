...
proposed to address this issue~\cite{DBLP:journals/iot/BittencourtISFM18}.
...
Software as a Service (SaaS)~\cite{DBLP:journals/cacm/ArmbrustFGJKKLPRSZ10}. IaaS provides users with remote access to computing power as a service, while PaaS offers a
...
Osmotic computing~\cite{DBLP:journals/computer/VillariFDRJR19} utilizes a concept known as a MEL (microelement) to encompass resources, services, and data. In the realm of IoT,
...
\begin{figure}[ht]
	...
	\caption[Osmotic computing architecture reference]{Osmotic computing architecture reference. Picture taken from~\Cite{DBLP:journals/computer/VillariFDRJR19}.}
...
The \emph{Dynamic Reconfigurable BIP} framework (DR-BIP)~\cite{DBLP:conf/isola/BallouliBBS18} includes three main aspects of dynamism: (I) the ability
...
available device or computer (fluid architectures~\cite{DBLP:conf/compsac/TaivalsaariMS14}).
The \emph{DR-BIP} framework is an extension of the \emph{Behavioral Interconnection Protocol} (BIP)~\cite{DBLP:journals/software/BasuBBCJNS11} and
\emph{Dy-BIP} (a former extension that support dynamic interactions)~\cite{DBLP:conf/soco/BozgaJMS12}.
...
motif, providing a flexible framework for coordination. The platform shares similarities with DReAM~\cite{DBLP:journals/sttt/NicolaMS20}, but the use of constraints
...
\begin{figure}[ht]
	...
	\caption[Motif-based systems]{Motif-based System Concept. Picture taken from~\cite{DBLP:conf/isola/BallouliBBS18}.}
...
this problem, a framework based on the pulverization approach is proposed~\cite{DBLP:journals/fi/CasadeiPPVW20}, which breaks the overall system behavior into tiny
...
The pulverization approach can be implemented in the framework of Aggregate Computing~\cite{DBLP:journals/computer/BealPV15}, where global self-organizing behavior
...
imposing a custom maximum temperature relative to the level of particulate matter (PM) in the area surrounding the household~\cite{DBLP:journals/fi/CasadeiPPVW20}.
...
Another initial research effort~\cite{DBLP:conf/acsos/AguzziCPSV21} was made by combining the pulverization approach with the multi-tier programming
paradigm~\cite{DBLP:journals/csur/WeisenburgerWS20}.
...
A language that supports multi-tier programming is \emph{ScalaLoci}~\cite{DBLP:journals/pacmpl/WeisenburgerKS18, DBLP:journals/programming/WeisenburgerS20}, a type-safe
...
The example provided in~\cite{DBLP:conf/acsos/AguzziCPSV21} shows how the pulverization approach can be fitted into the multi-tier programming paradigm, by defining
